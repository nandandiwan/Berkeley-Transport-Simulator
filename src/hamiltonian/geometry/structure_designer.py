...
import logging, numpy as np, scipy.spatial
from ..io.xyz import xyz2np
from ..base.aux import count_species, is_in_coords, print_dict
from ..base.abstract_interfaces import AbstractStructureDesigner
...
class StructDesignerXYZ(AbstractStructureDesigner):
    ...
    @property
    def num_of_nodes(self): return self._num_of_nodes
    @property
    def num_of_species(self): return self._num_of_species
    def get_neighbours(self, query):
        ans = self._get_neighbours(query)
        if ans[0][0] == np.inf: ans1 = []
        else: ans1 = [ans[1][0]]
...
class CyclicTopology(AbstractStructureDesigner):
    def __init__(self, primitive_cell_vectors, labels, coords, nn_distance):
        self._nn_distance = nn_distance; self.pcv = primitive_cell_vectors
        self.sizes = [np.linalg.norm(item) for item in self.pcv]
        self.interfacial_atoms_ind = []; self.virtual_and_interfacial_atoms = OrderedDict(); self.shift = np.zeros(3)
        ...
        logging.info("---------------------------------\n")
    @property
    def atom_list(self): return self.virtual_and_interfacial_atoms
    def _generate_atom_list(self, labels, coords):
        ...
        for item in distances1.T: self.shift += coords[np.argmin(item)]
        for j1, coord in enumerate(coords):
            for j2, basis_vec in enumerate(self.pcv):
                distances1[j1, j2] = np.inner(coord - self.shift, basis_vec) / self.sizes[j2]
                distances2[j1, j2] = np.inner(coord - self.shift - basis_vec, basis_vec) / self.sizes[j2]
        ...
        for j, item in enumerate(coords):
            if any(distances1[j]):
                self.virtual_and_interfacial_atoms.update({str(j) + "_" + labels[j]: item}); self.interfacial_atoms_ind.append(j)
